...
def main(args):
    ...
    if args.val:
        ...
        args.sem_seg = ""
        val_data_list = get_data_seg(args, tokenizer, clip_image_processor=processor, seg_nums=20, reasoning_data=True)
    ...
    if args.auto_resume and os.path.exists(resume_dir):
        args.resume = True
    else:
        args.resume = False
    if args.resume:
        load_path, client_state = model_engine.load_checkpoint(args.resume)
        resume_info = torch.load(os.path.join(args.resume, 'resume_info.pth'))
    ...
    for epoch in range(args.start_epoch, args.epochs+1):
        ...
        if (epoch % args.save_freq == 0 and epoch > 0) or epoch == args.epochs - 1:
...
def train(train_loader, model, epoch, global_step, scheduler, args):
    ...
    for step in range(args.steps_per_epoch):
        for i in range(args.grad_accumulation_steps):
            ...
            model.backward(loss)
            model.step()
            # except (torch.cuda.OutOfMemoryError, AssertionError, Exception) as e:
            #     print('Error occured:', e)
            #     print('conversation_list', input_dict['conversation_list'])
            #     for iii in range(len(input_dict['masks_list'])):
            #         print(input_dict['masks_list'][iii].shape)
...
def generate_ckpt_hf(init_ckpt_dir, ckpt_dir, cfg_file):
    ...
    for global_step in global_steps:
        ...
        cmd = [
            'CUDA_VISIBLE_DEVICES="" python merge_lora_weights_and_save_hf_model.py --cfg_file={} --weight="{}/pytorch_model.bin" --save_path="{}"'.format(
